...
async def start_command(client: Client, message: Message):
    ...
    buttons = [
        [InlineKeyboardButton("Developer 💀", url="https://t.me/E4Error")],
        [InlineKeyboardButton("Updates Channel ❣️", url="https://t.me/E4Error")],
        [InlineKeyboardButton("🔗 Source Code", url="https://t.me/E4Error")],
    ]
...
async def handle_settings_input(client: Client, message: Message):
    ...
    try:
        if state == "waiting_base_url":
            ...
            # Accept caption template as-is without reminders
...
async def callback_query_handler(client: Client, callback_query: CallbackQuery):
    ...
    if data == "close_settings":
        ...
        await callback_query.message.edit_text(
            ...
            f"Current:\n`{current_caption}`\n\n"
            "Available variables (optional): `{title}`, `{size}`, `{quality}`\n\n"
            "Example:\n`**🎬 {title}**\n\n**📦 {size} | 🎚 {quality}**\n\n**🔥 Uploaded By ~ @E4Error**`"
        )
